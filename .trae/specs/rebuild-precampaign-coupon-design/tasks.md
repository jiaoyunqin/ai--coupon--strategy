...
- [ ] Task 22: 重构促前规划的推荐灵感首屏。
  - [ ] SubTask 22.1: 保留轻量活动日历和参考促控制，将基于当前样本的推荐方向、结构、置信度和关键预估提升为首屏视觉焦点。
  - [ ] SubTask 22.2: 明确推荐内容为待共创候选，不触发真实预算、配置或发布。
  - [ ] SubTask 22.3: 将趋势、类目机会和历史实验洞察排列到推荐区域下方，并保留原有切换、下钻、来源和追问能力。
  - [ ] SubTask 22.4: 保持“促类型选择”删除状态，不修改“促前券档位设计”历史对话。

- [ ] Task 23: 实现促中调优双推荐方案与上下文共创。
  - [ ] SubTask 23.1: 首屏展示“当前波次人群与灵活渠道调整”和“下一波次方案设计”两个推荐模块。
  - [ ] SubTask 23.2: 两个模块分别提供独立追问和可点击展开详情，详情覆盖推荐结论、方案结构、推荐逻辑、风险与预估。
  - [ ] SubTask 23.3: 每个展开详情固定提供“基于此方案继续调整”按钮，创建独立对话并携带完整方案和当前活动上下文。
  - [ ] SubTask 23.4: 将既有筛选器、指标、竞对、实验、领券核销和投放矩阵排列在推荐方案下方，保留原交互。

- [ ] Task 24: 验证推荐优先信息架构与回归。
  - [ ] SubTask 24.1: 运行 JavaScript 语法、静态 ID、`git diff --check` 和目标文案检查。
  - [ ] SubTask 24.2: 浏览器验证促前推荐灵感、促中双方案、展开/收起、独立追问和“基于此方案继续调整”上下文。
  - [ ] SubTask 24.3: 验证原始数据模块位于推荐区域下方，桌面宽屏与 1280px 无横向溢出或遮挡。
  - [ ] SubTask 24.4: 回归“促前券档位设计”历史对话门禁、促中历史对话及原有图表交互。
