...
- [ ] Task 11: 重构促前首页数据与状态模型，改为活动与参考促集合驱动。
  - [ ] SubTask 11.1: 定义 26 年活动日历、活动日期、级别、阶段和历史关系。
  - [ ] SubTask 11.2: 实现“前序四次 + 同级别促 + 同名历史促”的自动参考样本算法和去重规则。
  - [ ] SubTask 11.3: 定义用户添加 / 删除参考促的状态，并同步样本覆盖与洞察上下文。
  - [ ] SubTask 11.4: 移除首页促类型、推荐方案、预算、ROI、具体 3+X/4+X、波次配置和巡检依赖。

- [ ] Task 12: 实现轻量活动下拉与参考促样本管理。
  - [ ] SubTask 12.1: 首页使用“26 年活动日历”下拉选择，不展示活动卡片墙。
  - [ ] SubTask 12.2: 展示当前活动名称、日期、级别、阶段和数据状态。
  - [ ] SubTask 12.3: 展示参考促标签及其“前序四次 / 同级别 / 同名历史 / 用户添加”原因。
  - [ ] SubTask 12.4: 支持删除参考促和从历史活动中添加参考促。
  - [ ] SubTask 12.5: 切换规划活动后自动重建参考促集合并更新洞察。

- [ ] Task 13: 实现细化的历史消费券实验洞察与来源说明。
  - [ ] SubTask 13.1: 展示活动前 15–20 天 GMV / DAC / 客单趋势及去年同期对比。
  - [ ] SubTask 13.2: 展示活动前窗口与历史活动期的分类目 GMV / DAC / 客单爆发情况。
  - [ ] SubTask 13.3: 展示各券档领取 / 核销趋势，并支持行业和价格带下钻。
  - [ ] SubTask 13.4: 展示分波次券档表现与券批次节奏。
  - [ ] SubTask 13.5: 展示价值、潜力、新客、回流等人群分层实验差异。
  - [ ] SubTask 13.6: 展示竞对券档、折扣、张数、频次和变化方向聚合。
  - [ ] SubTask 13.7: 每个章节包含证据、AI 小结、适用边界、来源和追问，不生成本期具体方案。
  - [ ] SubTask 13.8: 追问携带活动、参考促集合、洞察主题、证据和来源进入促前对话。

- [ ] Task 14: 验证活动、参考样本与历史洞察首页并执行回归。
  - [ ] SubTask 14.1: 运行 JavaScript 语法、静态 ID 和 diff 检查。
  - [ ] SubTask 14.2: 浏览器验证活动下拉、参考促自动生成、增删和活动切换重算。
  - [ ] SubTask 14.3: 验证大盘水位、类目爆发、券档、价格带、波次、人群和竞对洞察。
  - [ ] SubTask 14.4: 验证首页不再出现促类型、推荐 B、申请预算、预估 ROI、具体 4+X 方案和配置巡检。
  - [ ] SubTask 14.5: 验证历史促前对话、促中工作台和追问链路不受影响。
