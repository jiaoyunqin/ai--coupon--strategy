...
- [ ] Task 18: 修复跨年参考促时效权重与动态 24 月窗口。
  - [ ] SubTask 18.1: 以规划活动和参考促的实际日期间隔计算连续衰减权重，消除按自然年或日历分组造成的跨年跳变。
  - [ ] SubTask 18.2: 以当前规划活动日期为锚点动态计算向前 24 个月的参考窗口，排除窗口外及未来活动。
  - [ ] SubTask 18.3: 仅由用户添加的参考促使用中立时效权重；同时命中自动规则的活动保留自动原因及其时效权重，“用户添加”标记不额外增减权重。
  - [ ] SubTask 18.4: 补充跨年、闰年、恰好 24 个月、边界前后 1 天、未来日期和用户添加场景的权重与窗口测试。

- [ ] Task 19: 修复工作台回归与飞书 Mock 一致性。
  - [ ] SubTask 19.1: 新建对话时重置消息、报告、任务选择、定时任务选择和临时交互状态，避免继承上一对话内容。
  - [ ] SubTask 19.2: 将 `contextRange` 纳入对话状态持久化，确保重渲染、视图切换和返回对话后选值与实际上下文范围一致。
  - [ ] SubTask 19.3: 统一任务与定时任务的明细数组、列表渲染和计数来源，保证明细条目数、角标及汇总计数一致。
  - [ ] SubTask 19.4: 任务或定时任务数量为零时展示明确空态，不渲染残留明细、错误计数或不可用操作。
  - [ ] SubTask 19.5: 将飞书机器人群名称、群 ID、成员及关联信息统一替换为不可反推的脱敏 Mock，并保持列表与详情数据一致。
  - [ ] SubTask 19.6: 回归验证新建对话、`contextRange` 持久化、任务与定时任务明细/计数、零项空态、飞书群 Mock，以及促前/促中原有链路。
...
- [ ] Task 21: 收紧促前首页信息边界并修复洞察追问前缀。
  - [ ] SubTask 21.1: 将首页摘要限定为历史证据观察并标注“仅供本期分析参考”，不形成本期券方案、预算或执行动作。
  - [ ] SubTask 21.2: 规范洞察追问上下文，对 `insight.summary` 先移除已有 `AI 小结：` 前缀，确保上下文仅保留一个前缀。
  - [ ] SubTask 21.3: 浏览器回归七类历史洞察、洞察追问和“进入策略共创”入口，确认历史证据边界及既有交互可用。
  - [ ] SubTask 21.4: 运行 JavaScript 语法、静态 ID、目标文案和 diff 检查。
